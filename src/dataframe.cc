...
vector<GenJetRecoJetPair> DoStuff(
    unsigned int nJet,
    vfloat Jet_pt,
    vfloat Jet_eta,
    vfloat Jet_phi,
    ...
    for(unsigned int i=0; i < nGenJet; i++) {
        lvec_genjet.push_back(make_lv(GenJet_pt[i], GenJet_eta[i], GenJet_phi[i], GenJet_mass[i])); 
    }
...
void PrintPairs(vector<GenJetRecoJetPair>& pairs, GenJetRecoJetPair& buf, TTree& out_tree) {
    for (auto p : pairs) {
...
int main( int argc, char *argv[]) {
    TFile* file = TFile::Open("file://./data/ggh_hmumu/0C2B3A66-B042-E811-8C6D-44A8423DE2C0.root");
    ROOT::RDataFrame d("Events", file, {
    ...
    outfile->cd();
    unique_ptr<TTree> out_tree = make_unique<TTree>("GenJetRecoJetPair", "Gen - reco jet pair");
    GenJetRecoJetPair buf;
    (*out_tree).Branch("GenJet_pt", &buf.GenJet_pt, "GenJet_pt/F"); 
    
    cout << "Running RDataframe reduction..." << endl;
    d.Filter(IsGoodEvent).Define(
        "pairs", DoStuff
    ).Foreach(
        [&buf, &out_tree](vector<GenJetRecoJetPair> p){ PrintPairs(p, buf, *out_tree);}, {"pairs"}
    );
    out_tree->Write();
    outfile->Write();
    outfile->Close();
