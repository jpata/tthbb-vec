...
namespace MG5_sm__hgg_plugin_full 
{
    ...
    std::complex<double> COUP, double M3, double W3, std::complex<double> S3[])
{
  ...
  P3[2] = -S3[1].imag(); 
  P3[3] = -S3[0].imag(); 
  TMP7 = (F1[2] * F2[2] + F1[3] * F2[3]); 
  TMP8 = (F1[4] * F2[4] + F1[5] * F2[5]); 
  denom = COUP/((P3[0] * P3[0]) - (P3[1] * P3[1]) - (P3[2] * P3[2]) - (P3[3] *
      P3[3]) - M3 * (M3 - cI * W3));
  S3[2] = denom * (+cI * (TMP7 + TMP8)); 
}
    ...
    & vertex)
{
  ...
  vertex = COUP * S3[2] * (-cI * (TMP3 * TMP4) + cI * (TMP5 * TMP6)); 
}
...
}  // end namespace $(namespace)s_sm__hgg_plugin_full
